const {
  cunk, RGBA, PETL, gifPETL, gifRGBA
} = require ('./utils');

exports.pngToApngRGBA = async(imagesBuffer) => {
...
exports.pngToApngPLTE = async(imagesBuffer) => {
  ...
  const height = imgBuffer.height; // 高度
  const bitDepth = 8; // 深度，通常8
  const colorType = 3; // 類型，通常 6（真彩色）  PLTE 為3
  const compressionMethod = 0; // 压缩方法，通常为 0（deflate/inflate 压缩）
  const filterMethod = 0; // 滤波器方法，通常为 0（adaptive filtering）
  const interlaceMethod = 0; // 隔行扫描方法，通常为 0（不使用隔行扫描）
  const ihdr = Buffer.alloc (13); // 建立 Buffer
  ...
  const PLTECunk = new cunk ('PLTE', Buffer.concat (imgBuffer.PLTE));
    
  // step 5 create tRNS 由png取得
  const tRNSCunk = new cunk ('tRNS', Buffer.concat ( imgBuffer.tRNS));
  ...
  const apngData = Buffer.concat ([pngSignature,
    ihdrChunk.getBuffer (),
    acTLCunk.getBuffer (),
    PLTECunk.getBuffer (),
    tRNSCunk.getBuffer (),
...
exports.gifToApngPLTE = async(imagesBuffer) => {
  ...
  const delayDen = 100; // 延遲時間分母
  const disposeOp = 0; // Dispose 操作    0不進行處理 1完全清除 2 渲染回上一個
  const blendOp = 1; // Blend 操作        0覆蓋所有顏色  1混和顏色
...
exports.gifToApngRGBA = async(imagesBuffer) => {
