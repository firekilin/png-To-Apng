const fs = require ('fs');
const { cunk, pngToApngPETL } = require ('./utils');
...
let run = async() => {
  ...
  const colorType = 3; // 類型，通常 6（真彩色） 若為apng 為3
  ...
  let imgBuffer = await pngToApngPETL (imagesBuffer);
  console.log (imgBuffer.PLTE);
  console.log (imgBuffer.tRNS);
  ...
  //console.log (tRNSCunk.getBuffer ());
